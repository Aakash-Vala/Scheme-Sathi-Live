    ...
    container.innerHTML = `
      <div class="auth-buttons-group">
        <a href="/login?tab=login" class="btn-auth-nav btn-auth-login" onclick="if(window.location.pathname !== '/login'){ event.preventDefault(); openAuthModal('login'); }">
          ...
          <span data-i18n="nav_login">${t('nav_login') || 'Login'}</span>
        </a>
        <a href="/login?tab=signup" class="btn-auth-nav btn-auth-signup" onclick="if(window.location.pathname !== '/login'){ event.preventDefault(); openAuthModal('signup'); }">
          ...
          <span data-i18n="nav_signup">${t('nav_signup') || 'Sign Up'}</span>
        </a>
      </div>
    `;
  }
